...
## 1. Introduction
   ...
   The Hypertext Transfer Protocol (HTTP) [RFC7230](https://www.rfc-editor.org/rfc/rfc7230.html) provides compatible
   ...
   The WebSocket Protocol [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html) uses the HTTP/1.1 Upgrade mechanism
   (Section 6.7 of [RFC7230](https://www.rfc-editor.org/rfc/rfc7230.html)) to transition a TCP connection from HTTP
   into a WebSocket connection.  A different approach must be taken with
   HTTP/2 [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html).  Due to its multiplexing nature, HTTP/2 does not
   ...
   Protocols) response code.  These are all required by the [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html)
   opening handshake.
   ...
   HTTP/2 in Section 8.3 of [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html).  The extension allows the
...
## 2. Terminology
   ...
   BCP 14 [RFC2119](https://www.rfc-editor.org/rfc/rfc2119.html) [RFC8174](https://www.rfc-editor.org/rfc/rfc8174.html) when, and only when, they appear in all
...
## 3. The SETTINGS_ENABLE_CONNECT_PROTOCOL SETTINGS Parameter
   ...
   [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html), Section 6.5.2.
   ...
   of [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html).  Specifically, the addition a new pseudo-header field,
   ...
   (Section 8.1.2.6 of [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html)).

## 4. The Extended CONNECT Method

   Usage of the CONNECT method in HTTP/2 is defined by Section 8.3 of
   [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html).  This extension modifies the method in the following ways:
   ...
   o  On requests bearing the :protocol pseudo-header field, the
      ...
      Section 8.1.2.3 of [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html) instead of Section 8.3 of that
...
## 5. Using Extended CONNECT to Bootstrap the WebSocket Protocol
   ...
   request replaces the GET-based request in [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html) and is used to
   process the WebSockets opening handshake.

   The scheme of the target URI (Section 5.1 of [RFC7230](https://www.rfc-editor.org/rfc/rfc7230.html)) MUST be
   ...
   connection used by this specification are established by [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html)
   for https requests and [RFC8164](https://www.rfc-editor.org/rfc/rfc8164.html) for http requests.

   [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html) requires the use of Connection and Upgrade header fields
   that are not part of HTTP/2.  They MUST NOT be included in the
   CONNECT request defined here.

   [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html) requires the use of a Host header field that is also not
   ...
   Accept header fields of [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html) as that functionality has been
   superseded by the :protocol pseudo-header field.

   The Origin [RFC6454](https://www.rfc-editor.org/rfc/rfc6454.html), Sec-WebSocket-Version, Sec-WebSocket-Protocol,
   and Sec-WebSocket-Extensions header fields are used in the CONNECT
   request and response-header fields as defined in [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html).  Note
   ...
   proceed with the WebSocket Protocol [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html) using the HTTP/2 stream
   from the CONNECT transaction as if it were the TCP connection
   referred to in [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html).  The state of the WebSocket connection at
   this point is OPEN, as defined by [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html), Section 4.1.

   The HTTP/2 stream closure is also analogous to the TCP connection
   closure of [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html).  Orderly TCP-level closures are represented as
   END_STREAM flags ([RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html), Section 6.1).  RST exceptions are
   represented with the RST_STREAM frame ([RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html), Section 6.4) with
   the CANCEL error code ([RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html), Section 7).
...
## 8. Security Considerations
   ...
   [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html) ensures that non-WebSockets clients, especially
   ...
   The security considerations of [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html), Section 10 continue to
...
### 9.1. A New HTTP/2 Setting
   ...
   that was established by Section 11.3 of [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html).
...
### 9.2. A New HTTP Upgrade Token
   ...
   registry that was established by [RFC7230](https://www.rfc-editor.org/rfc/rfc7230.html).

      Value: websocket
      Description: The Web Socket Protocol
      Expected Version Tokens:
      References: [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html) [RFC8441](https://www.rfc-editor.org/rfc/rfc8441.html)
...
## 10. Normative References
   ...
   [RFC6455](https://www.rfc-editor.org/rfc/rfc6455.html)  Fette, I. and A. Melnikov, "The WebSocket Protocol",
              RFC 6455, DOI 10.17487/RFC6455, December 2011,
              <https://www.rfc-editor.org/info/rfc6455>.

   [RFC7230](https://www.rfc-editor.org/rfc/rfc7230.html)  Fielding, R., Ed. and J. Reschke, Ed., "Hypertext Transfer
              Protocol (HTTP/1.1): Message Syntax and Routing",
              RFC 7230, DOI 10.17487/RFC7230, June 2014,
              <https://www.rfc-editor.org/info/rfc7230>.

   [RFC7540](https://www.rfc-editor.org/rfc/rfc7540.html)  Belshe, M., Peon, R., and M. Thomson, Ed., "Hypertext
              Transfer Protocol Version 2 (HTTP/2)", RFC 7540,
              DOI 10.17487/RFC7540, May 2015,
              <https://www.rfc-editor.org/info/rfc7540>.

   [RFC8164](https://www.rfc-editor.org/rfc/rfc8164.html)  Nottingham, M. and M. Thomson, "Opportunistic Security for
              HTTP/2", RFC 8164, DOI 10.17487/RFC8164, May 2017,
              <https://www.rfc-editor.org/info/rfc8164>.

   [RFC8174](https://www.rfc-editor.org/rfc/rfc8174.html)  Leiba, B., "Ambiguity of Uppercase vs Lowercase in RFC
