...
### 1.1. Background
   ...
   HTTP calls [RFC6202](https://www.rfc-editor.org/rfc/rfc6202.html).
   ...
   meant to be used for bidirectional communication (see [RFC6202](https://www.rfc-editor.org/rfc/rfc6202.html) for
...
### 1.2. Protocol Overview
   ...
   Request-Line and Status-Line productions are defined in [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).
   ...
   present, such as cookies [RFC6265](https://www.rfc-editor.org/rfc/rfc6265.html).  The format and parsing of
   headers is as defined in [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).
   ...
   types for textual data (which is interpreted as UTF-8 [RFC3629](https://www.rfc-editor.org/rfc/rfc3629.html)
...
### 1.3. Opening Handshake
   ...
   In compliance with [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html), header fields in the handshake may be
   sent by the client in any order, so the order in which different
   header fields are received is not significant.

   The "Request-URI" of the GET method [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html) is used to identify the
...
```
   ...
   handshake as per [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html), so that both the client and the server
...
```
   ...
   The |Origin| header field [RFC6454](https://www.rfc-editor.org/rfc/rfc6454.html) is used to protect against
```
...
```
   ...
   in the header field, e.g., the base64-encoded [RFC4648](https://www.rfc-editor.org/rfc/rfc4648.html) version minus
   any leading and trailing whitespace) and concatenate this with the
   Globally Unique Identifier (GUID, [RFC4122](https://www.rfc-editor.org/rfc/rfc4122.html)) "258EAFA5-E914-47DA-
   95CA-C5AB0DC85B11" in string form, which is unlikely to be used by
   network endpoints that do not understand the WebSocket Protocol.  A
   SHA-1 hash (160 bits) [FIPS.180-3], base64-encoded (see Section 4 of
   [RFC4648](https://www.rfc-editor.org/rfc/rfc4648.html)), of this concatenation is then returned in the server's
   handshake.
...
```
   ...
   then base64-encoded (see Section 4 of [RFC4648](https://www.rfc-editor.org/rfc/rfc4648.html)), to give the value
...
```
   ...
   cookies, as described in [RFC6265](https://www.rfc-editor.org/rfc/rfc6265.html).
...
### 1.6. Security Model
   ...
   servers of pre-existing protocols like SMTP [RFC5321](https://www.rfc-editor.org/rfc/rfc5321.html) and HTTP, while
...
### 1.7. Relationship to TCP and HTTP
   ...
   Transport Layer Security (TLS) [RFC2818](https://www.rfc-editor.org/rfc/rfc2818.html).
...
### 2.1. Terminology and Other Conventions
   ...
   notational formats as defined in STD 63 [RFC3629](https://www.rfc-editor.org/rfc/rfc3629.html).
...
```
   ...
   The term "URI" is used in this document as defined in [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html).
   ...
   Note that this document uses both [RFC5234](https://www.rfc-editor.org/rfc/rfc5234.html) and [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html) variants of
   ABNF in different sections.

## 3. WebSocket URIs

   This specification defines two URI schemes, using the ABNF syntax
   defined in RFC 5234 [RFC5234](https://www.rfc-editor.org/rfc/rfc5234.html), and terminology and ABNF productions
   defined by the URI specification RFC 3986 [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html).

          ws-URI = "ws:" "//" host [ ":" port ] path [ "?" query ]
          wss-URI = "wss:" "//" host [ ":" port ] path [ "?" query ]

          host = <host, defined in [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html), Section 3.2.2>
          port = <port, defined in [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html), Section 3.2.3>
          path = <path-abempty, defined in [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html), Section 3.3>
          query = <query, defined in [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html), Section 3.4>
...
### 4.1. Client Requirements
   ...
   3.  _Proxy Usage_: If the client is configured to use a proxy when
       ...
       proxies are encouraged to use a SOCKS5 [RFC1928](https://www.rfc-editor.org/rfc/rfc1928.html) proxy for
   ...
   5.  If /secure/ is true, the client MUST perform a TLS handshake over
       ...
       the handshake data [RFC2818](https://www.rfc-editor.org/rfc/rfc2818.html).  If this fails (e.g., the server's
       ...
       encrypted tunnel [RFC5246](https://www.rfc-editor.org/rfc/rfc5246.html).

       Clients MUST use the Server Name Indication extension in the TLS
       handshake [RFC6066](https://www.rfc-editor.org/rfc/rfc6066.html).
...
```
        ...
        been base64-encoded (see Section 4 of [RFC4648](https://www.rfc-editor.org/rfc/rfc4648.html)).  The nonce
...
```
        ...
        [RFC6454](https://www.rfc-editor.org/rfc/rfc6454.html) if the request is coming from a browser client.  If
        ...
        running.  See [RFC6454](https://www.rfc-editor.org/rfc/rfc6454.html) for the details of how this header field
...
```
        ...
        [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html) and MUST all be unique strings.  The ABNF for the
        value of this header field is 1#token, where the definitions of
        constructs and rules are as given in [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).
...
```
        cookies [RFC6265](https://www.rfc-editor.org/rfc/rfc6265.html) and/or authentication-related header fields
        such as the |Authorization| header field [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html), which are
```
   ...
   1.  If the status code received from the server is not 101, the
       client handles the response per HTTP [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html) procedures.  In
...
```
   ...
   Please note that according to [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html), all header field names in
...
```
   ...
   that cookies should be set (as defined by [RFC6265](https://www.rfc-editor.org/rfc/rfc6265.html)), these cookies
...
#### 4.2.1. Reading the Client's Opening Handshake
   ...
   per [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html), the order of the header fields is not important),
...
```
        ...
        [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html) that should be interpreted as a /resource name/
...
```
        ...
        Section 4 of [RFC4648](https://www.rfc-editor.org/rfc/rfc4648.html)) value that, when decoded, is 16 bytes in
        length.
...
```
   ...
   10.  Optionally, other header fields, such as those used to send
        ...
        fields are ignored, as per [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).
...
#### 4.2.2. Sending the Server's Opening Handshake
   ...
   1.  If the connection is happening on an HTTPS (HTTP-over-TLS) port,
       ...
       through the encrypted tunnel [RFC5246](https://www.rfc-editor.org/rfc/rfc5246.html).
...
```
       ...
       |WWW-Authenticate| header field as described in [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).

```

   3.  The server MAY redirect the client using a 3xx status code
       [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).  Note that this step can happen together with, before,
...
```
       ...
       /resource name/
          ...
          in the "Request-URI" [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html) of the GET method.  If the
...
```
          ...
          rules are as given in [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).
...
```
   ...
   5.  If the server chooses to accept the incoming connection, it MUST
       ...
       1.  A Status-Line with a 101 response code as per RFC 2616
           [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).  Such a response could look like "HTTP/1.1 101
           Switching Protocols".

```

       2.  An |Upgrade| header field with value "websocket" as per RFC
           2616 [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).
...
```
           ...
           encoding (see Section 4 of [RFC4648](https://www.rfc-editor.org/rfc/rfc4648.html)) this 20-byte hash.

           The ABNF [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html) of this header field is defined as
           follows:
...
### 4.3. Collected ABNF for New Header Fields Used in Handshake
   ...
   [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html), including the "implied *LWS rule".
...
### 5.2. Base Framing Protocol
   ...
   [RFC5234](https://www.rfc-editor.org/rfc/rfc5234.html) given in detail in this section.  (Note that, unlike in
...
```
   ...
   [RFC5234](https://www.rfc-editor.org/rfc/rfc5234.html).  It is important to note that the representation of this
   ...
   values.  [RFC5234](https://www.rfc-editor.org/rfc/rfc5234.html) does not specify a character encoding: "Rules
...
### 5.3. Client-to-Server Masking
   ...
   the bytes that appear on the wire.  RFC 4086 [RFC4086](https://www.rfc-editor.org/rfc/rfc4086.html) discusses what
...
#### 7.4.1. Defined Status Codes
   ...
   1007
      ...
      consistent with the type of the message (e.g., non-UTF-8 [RFC3629](https://www.rfc-editor.org/rfc/rfc3629.html)
...
### 9.1. Negotiating Extensions
   ...
   header fields (see [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html), Section 4.2) and the value of the
   header field is defined by the following ABNF [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).  Note that
   this section is using ABNF syntax/rules from [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html), including the
...
### 10.6. Connection Confidentiality and Integrity
   ...
   algorithms.  [RFC5246](https://www.rfc-editor.org/rfc/rfc5246.html) provides additional guidance in Appendix A.5
   and Appendix D.3.
...
### 10.7. Handling of Invalid Data
   ...
   SHOULD return an appropriate HTTP [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html) status code.
...
### 10.8. Use of SHA-1 by the WebSocket Handshake
   ...
   [RFC4270](https://www.rfc-editor.org/rfc/rfc4270.html)).
...
```
   ...
   URI scheme syntax
      Using the ABNF [RFC5234](https://www.rfc-editor.org/rfc/rfc5234.html) syntax and ABNF terminals from the URI
      specification [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html):

           "ws:" "//" authority path-abempty [ "?" query ]

   The <path-abempty> and <query> [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html) components form the resource
   name sent to the server to identify the kind of service desired.
   Other components have the meanings described in [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html).
   ...
   Encoding considerations
      ...
      in [RFC3987](https://www.rfc-editor.org/rfc/rfc3987.html) or its replacement.  For the purposes of scheme-based
      ...
      equivalent (see Section 5.3.3 of [RFC3987](https://www.rfc-editor.org/rfc/rfc3987.html)).
      ...
      the URI [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html) and Internationalized Resource Identifier (IRI)
      [RFC3987](https://www.rfc-editor.org/rfc/rfc3987.html) specifications.
...
```
   ...
   components have the meanings described in [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html).
   ...
   Encoding considerations
      ...
      5.3.3 of [RFC3987](https://www.rfc-editor.org/rfc/rfc3987.html)).

      Characters in other components that are excluded by the syntax
      defined above MUST be converted from Unicode to ASCII by first
      encoding the characters as UTF-8 and then replacing the
      corresponding bytes using their percent-encoded form as defined in
      the URI [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html) and IRI [RFC3987](https://www.rfc-editor.org/rfc/rfc3987.html) specifications.
...
### 11.2. Registration of the "WebSocket" HTTP Upgrade Keyword
   ...
   Registry as per RFC 2817 [RFC2817](https://www.rfc-editor.org/rfc/rfc2817.html).
...
#### 11.3.1. Sec-WebSocket-Key
   ...
   Message Header Field Names registry [RFC3864](https://www.rfc-editor.org/rfc/rfc3864.html).
...
#### 11.3.2. Sec-WebSocket-Extensions
   ...
   Permanent Message Header Field Names registry [RFC3864](https://www.rfc-editor.org/rfc/rfc3864.html).
...
### 11.4. WebSocket Extension Name Registry
   ...
   with the principles set out in RFC 5226 [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
...
```
   ...
   Served" IANA registration policy [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
...
### 11.5. WebSocket Subprotocol Name Registry
   ...
   accordance with the principles set out in RFC 5226 [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
...
```
      ...
      the value must be a token as defined by RFC 2616 [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html).
   ...
   First Served" IANA registration policy [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
...
### 11.6. WebSocket Version Number Registry
   ...
   principles set out in RFC 5226 [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
...
```
   ...
   subject to the "IETF Review" IANA registration policy [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
   ...
   to the "Expert Review" IANA registration policy [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html), with the
...
### 11.7. WebSocket Close Code Number Registry
   ...
   out in RFC 5226 [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
...
### 11.8. WebSocket Opcode Registry
   ...
   in accordance with the principles set out in RFC 5226 [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
   ...
   registration policy [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
...
### 11.9. WebSocket Framing Header Bits Registry
   ...
   [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).  This registry controls assignment of the bits marked
   ...
   "Standards Action" IANA registration policy [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html).
...
## 12. Using the WebSocket Protocol from Other Specifications
   ...
   o  An ASCII serialization of an origin [RFC6454](https://www.rfc-editor.org/rfc/rfc6454.html) that is being made
...
### 14.1. Normative References
   ...
   [RFC1928](https://www.rfc-editor.org/rfc/rfc1928.html)  Leech, M., Ganis, M., Lee, Y., Kuris, R., Koblas, D., and
              L. Jones, "SOCKS Protocol Version 5", RFC 1928,
              March 1996.

   [RFC2119](https://www.rfc-editor.org/rfc/rfc2119.html)  Bradner, S., "Key words for use in RFCs to Indicate
              Requirement Levels", BCP 14, RFC 2119, March 1997.

   [RFC2616](https://www.rfc-editor.org/rfc/rfc2616.html)  Fielding, R., Gettys, J., Mogul, J., Frystyk, H.,
              Masinter, L., Leach, P., and T. Berners-Lee, "Hypertext
              Transfer Protocol -- HTTP/1.1", RFC 2616, June 1999.

   [RFC2817](https://www.rfc-editor.org/rfc/rfc2817.html)  Khare, R. and S. Lawrence, "Upgrading to TLS Within
              HTTP/1.1", RFC 2817, May 2000.

   [RFC2818](https://www.rfc-editor.org/rfc/rfc2818.html)  Rescorla, E., "HTTP Over TLS", RFC 2818, May 2000.

   [RFC3629](https://www.rfc-editor.org/rfc/rfc3629.html)  Yergeau, F., "UTF-8, a transformation format of ISO
              10646", STD 63, RFC 3629, November 2003.

   [RFC3864](https://www.rfc-editor.org/rfc/rfc3864.html)  Klyne, G., Nottingham, M., and J. Mogul, "Registration
              Procedures for Message Header Fields", BCP 90, RFC 3864,
              September 2004.

   [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html)  Berners-Lee, T., Fielding, R., and L. Masinter, "Uniform
              Resource Identifier (URI): Generic Syntax", STD 66,
              RFC 3986, January 2005.

   [RFC3987](https://www.rfc-editor.org/rfc/rfc3987.html)  Duerst, M. and M. Suignard, "Internationalized Resource
              Identifiers (IRIs)", RFC 3987, January 2005.

   [RFC4086](https://www.rfc-editor.org/rfc/rfc4086.html)  Eastlake, D., Schiller, J., and S. Crocker, "Randomness
              Requirements for Security", BCP 106, RFC 4086, June 2005.

   [RFC4648](https://www.rfc-editor.org/rfc/rfc4648.html)  Josefsson, S., "The Base16, Base32, and Base64 Data
              Encodings", RFC 4648, October 2006.

   [RFC5226](https://www.rfc-editor.org/rfc/rfc5226.html)  Narten, T. and H. Alvestrand, "Guidelines for Writing an
              IANA Considerations Section in RFCs", BCP 26, RFC 5226,
              May 2008.

   [RFC5234](https://www.rfc-editor.org/rfc/rfc5234.html)  Crocker, D. and P. Overell, "Augmented BNF for Syntax
              Specifications: ABNF", STD 68, RFC 5234, January 2008.

   [RFC5246](https://www.rfc-editor.org/rfc/rfc5246.html)  Dierks, T. and E. Rescorla, "The Transport Layer Security
              (TLS) Protocol Version 1.2", RFC 5246, August 2008.

   [RFC6066](https://www.rfc-editor.org/rfc/rfc6066.html)  Eastlake, D., "Transport Layer Security (TLS) Extensions:
              Extension Definitions", RFC 6066, January 2011.

   [RFC6454](https://www.rfc-editor.org/rfc/rfc6454.html)  Barth, A., "The Web Origin Concept", RFC 6454,
              December 2011.

### 14.2. Informative References

   [RFC4122](https://www.rfc-editor.org/rfc/rfc4122.html)  Leach, P., Mealling, M., and R. Salz, "A Universally
              Unique IDentifier (UUID) URN Namespace", RFC 4122,
              July 2005.

   [RFC4270](https://www.rfc-editor.org/rfc/rfc4270.html)  Hoffman, P. and B. Schneier, "Attacks on Cryptographic
              Hashes in Internet Protocols", RFC 4270, November 2005.

   [RFC5321](https://www.rfc-editor.org/rfc/rfc5321.html)  Klensin, J., "Simple Mail Transfer Protocol", RFC 5321,
              October 2008.

   [RFC6202](https://www.rfc-editor.org/rfc/rfc6202.html)  Loreto, S., Saint-Andre, P., Salsano, S., and G. Wilkins,
              "Known Issues and Best Practices for the Use of Long
              Polling and Streaming in Bidirectional HTTP", RFC 6202,
              April 2011.

   [RFC6265](https://www.rfc-editor.org/rfc/rfc6265.html)  Barth, A., "HTTP State Management Mechanism", RFC 6265,
              April 2011.
